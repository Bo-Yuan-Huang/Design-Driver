...
import sys
from uc8051ast import Ctx8051, Ctx8051FromSyn, create8051Inputs
...
def synthesize():
    ...
    PC_jc = If(Equal(ctx.CY(), Choice('jc_polarity', op0, [BitVecVal(1,1), BitVecVal(0,1)])), PC_jc_taken, PC_jc_seq)
    ...
    cjne_src1 = Choice('cjne_src1', op0, [ctx.ACC, ReadMem(ctx.IRAM, ctx.Rx(0)), ReadMem(ctx.IRAM, ctx.Rx(1))] + ctx.Rxs())
    cjne_src2 = Choice('cjne_src2', op0, [op1, op2, ctx.readDirect(Choice('cjne_iram_addr', op0, [op1, op2]))])
    ...
    djnz_src = Choice('djnz_src', op0, [ctx.readDirect(Choice('djnz_iram_src', op0, [op1, op2]))] + ctx.Rxs())

...
#        print "".join("{:02X}".format(ord(c)) for c in self.key.translate(trans_36))
#        sha.update(message)
#        sha2 = hashlib.sha1()
...
#        print "".join("{:02X}".format(ord(c)) for c in self.key.translate(trans_5C))
...
def sign(data):
    ...
    #print "hash"
    #print "".join("{:02X}".format(ord(c)) for c in hashed)
...
def verifySig(data, sig):
    ...
    #trans_36 = "".join ([chr (x ^ 0x36) for x in xrange(256)])
    #sha = hashlib.sha1()
    #sha.update(SIGNSEED.translate(trans_36)+chr(0x36)*(64-len(SIGNSEED)))
    #print "".join("{:02X}".format(ord(c)) for c in SIGNSEED.translate(trans_36))
    #sha.update(data)
    #sha2 = hashlib.sha1()
    #sha2.update(SIGNSEED.translate(trans_5C)+chr(0x5C)*(64-len(SIGNSEED)))
    #sha2.update(sha.digest())
    #print "".join("{:02X}".format(ord(c)) for c in sha.digest())

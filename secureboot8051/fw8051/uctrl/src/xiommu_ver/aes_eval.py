...
AesState = namedtuple('AesState', [
    'state', 'addr', 'length', 'num_op_bytes', 
...
def dump_aes_state(fileobject, s_in):
    ...
    s1 = [hexstr(x) for x in [s_in.state, s_in.addr, s_in.length, s_in.num_op_bytes]]
...
def eval_aes(s_in):
    with tempfile.NamedTemporaryFile() as fileobject:
        ...
        ctr = [hex_int(x) for x in words[5:5+16]]
        key = [hex_int(x) for x in words[5+16:5+32]]

        sz = hex_int(words[37])
        defval = hex_int(words[38])

        mem = []
        assert len(words) == 39 + 2*sz
        for i in xrange(39, 39+2*sz, 2):
            a = hex_int(words[i])
            d = hex_int(words[i+1])
            mem.append([a, d])
        mem.append(defval)
        xram = mem
        s_out = AesState(state, addr, length, num_op_bytes,  ctr, key, xram, 0, 0, 0)
...
def test_eval():
    ...
    addr_in = 0xFF00
    data_in = 1
    s_in = AesState(state, addr,length, num_op_bytes, ctr, key, xram, op, addr_in, data_in)
